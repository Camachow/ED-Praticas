...
bool a_star(const Graph &graph, double s, int q)
{
    ...
    pq.push(State(0.0, graph.euclideanDistance(0, n - 1), 0, 0));
    ...
    while (!pq.empty())
    {
        ...
        for (size_t i = 0; i < graph.getAdjList(u).size(); ++i)
        {
            ...
            if (new_cost < dist[v][portals_used])
            {
                ...
                pq.push(State(new_cost, new_cost + graph.euclideanDistance(v, n - 1), v, portals_used));
            }
        }
        ...
        if (portals_used < q)
        {
            for (size_t i = 0; i < graph.getPortals(u).size(); ++i)
            {
                ...
                if (cost < dist[v][portals_used + 1])
                {
                    ...
                    pq.push(State(cost, cost + graph.euclideanDistance(v, n - 1), v, portals_used + 1));
                }
            }
        }
    }
